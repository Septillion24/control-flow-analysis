...
def generate_data_list_from_binaries(binary_dir:str = './binaries', ) -> list[Data]:
    ...
    for binary_file in os.listdir(f"{binary_dir}/malware"):
        data = preprocess_binary(f"{binary_dir}/malware/{binary_file}")
        data.y = 1
        data_list.append(data)

    for binary_file in os.listdir(f"{binary_dir}/benign"):
        data = preprocess_binary(f"{binary_dir}/benign/{binary_file}")
        data.y = 0
        data_list.append(data)
    
    return data_list

def get_train_test_split(data_list: list) -> Tuple[list[Data], list[Data]]:
